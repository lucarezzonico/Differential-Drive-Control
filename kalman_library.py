...
def get_orientation_error(robot_orientation, robot_pos, last_goal, next_goal):
    ...
    alpha_r = math.atan2(next_goal[1] - robot_pos[1], next_goal[0] - robot_pos[0])
    orientation_angle = robot_orientation - trajectory_orientation + (trajectory_orientation - alpha_r)
...
def calculate_next_kalman_state(is_thymio_visible, _kalman_state, _thymio_info, u, dt, _sigma, _filter):
    ...
    if (is_thymio_visible):
        ...
        R = np.diag([10000000, 10000000, 10000000])

    _filter.F = jacobian_motion(_kalman_state, u, dt)

    _filter.H = H    # Measurement function
    _filter.P = _sigma                 # covariance matrix
    _filter.R = R                      # state uncertainty
    _filter.Q = Q # process uncertainty

    _filter.predict()
    _filter.update([*_thymio_info.get_position(), _thymio_info.get_yaw()])
    x = _filter.x

    return Kalman_State(int(x[0]), int(x[1]), x[2]), _filter.P,
...
def main():
    ...
    my_filter.x = np.array([*thymio_info.get_position(), thymio_info.get_yaw()])# initial state (location and yaw)
    ...
    while True:
        ...
        if is_obstacle_detected():
            ...
            print("Yaw: " + str(math.degrees(kalman_state.get_yaw())) + " Position: " + str(
                kalman_state.get_position()) + " Speed: " + str(u))
