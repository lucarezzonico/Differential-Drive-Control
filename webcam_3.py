...
while True:
    ...
    thymio = Thymio_Info(*(int(thymio_center_x), int(thymio_center_y)), enc_radius, yaw)
    ...
    if (full_analysis):
        ...
        for cnt in corner_contours:
            area = cv2.contourArea(corner_contours)
            if (area > 300):
    ...
    print("Yaw: "+str(math.degrees(thymio.get_yaw()))+" Position: "+str(thymio.get_position()))

    cv2.imshow("Multiple Color Detection in Real-TIme", img2)
    c = cv2.waitKey(1)
    if c == 27:
        break
    time.sleep(1)
